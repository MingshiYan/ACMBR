...
class Trainer(object):
    ...
    def clear_parameter(self, model):
        ...
        confound_graphs = getattr(model, 'confound_graphs', None)
        if confound_graphs is not None:
            for tmp in confound_graphs:
                tmp.user_centroids = None
                tmp.user_2cluster = None
                tmp.item_centroids = None
                tmp.item_2cluster = None
