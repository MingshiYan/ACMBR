...
class ACMBR(nn.Module):
    ...
    def _construct_graphs(self):
        ...
        self.target_graph = NCL(self.n_users + 1, self.n_items + 1, target_inter_matrix, self.device, self.embedding_size, self.layers, self.ssl_tau, self.ssl_weight)
        target_inter_matrix_bool = target_inter_matrix.astype(bool)

        for i in range(len(self.behaviors) - 1):
            self.confound_graphs.append(NCL(self.n_users + 1, self.n_items + 1, self.inter_matrix[i], self.device, self.embedding_size, self.layers, self.ssl_tau, self.ssl_weight))
            ...
            self.condition_graphs.append(NCL(self.n_users + 1, self.n_items + 1, tmp_target_inter_matrix, self.device, self.embedding_size, self.layers, self.ssl_tau, self.ssl_weight))
    ...
    def forward(self, batch_data):
        ...
        for idx in range(len(self.confound_graphs)):
            # 计算混淆概率
            tmp_loss, tmp_confound_user_all_embedding, tmp_confound_item_all_embedding = self.confound_graphs[idx].calculate(all_embeddings, user.long().squeeze(), p_item.long().squeeze(), n_item.long().squeeze())
            aux_loss.append(tmp_loss)

            # 计算条件概率
            tmp_condition_user_all_embedding, tmp_condition_item_all_embedding, _ = self.condition_graphs[idx](all_embeddings)
    ...
    def full_predict(self, users):
        if self.storage_user_embeddings is None:
            ...
            for idx in range(len(self.confound_graphs)):
                # 计算混淆概率
                tmp_confound_user_all_embedding, tmp_confound_item_all_embedding, _ = self.confound_graphs[idx](all_embeddings)
