...
class SGL(nn.Module):
    ...
    def __init__(self, user_count, item_count, inter_matrix, device, embedding_size, layers, ssl_tau=0.5, ssl_weight=0.03):
        super(SGL, self).__init__()
        self._user = inter_matrix.row
        self._item = inter_matrix.col
    ...
    def forward(self, graph, main_ego):
        ...
        all_ego = torch.sum(all_ego, dim=1, keepdim=False)
        user_emd, item_emd = torch.split(all_ego, [self.n_users, self.n_items], dim=0)

        return user_emd, item_emd

    def calculate(self, all_embeddings, users=None, p_items=None, n_items=None, if_calculate_loss=True):

        user_emd, item_emd = self.forward(self.train_graph, all_embeddings)
        if if_calculate_loss:
            user_sub1, item_sub1 = self.forward(self.sub_graph1, all_embeddings)
            user_sub2, item_sub2 = self.forward(self.sub_graph2, all_embeddings)
            bpr_loss = self.calc_bpr_loss(user_emd, item_emd, users, p_items, n_items)
            ssl_loss = self.calc_ssl_loss(users, p_items, user_sub1, user_sub2, item_sub1, item_sub2)
            total_loss = bpr_loss + ssl_loss
            return total_loss, user_emd, item_emd
        else:
            return user_emd, item_emd
    ...
    #         self.restore_user_e, self.restore_item_e = self.forward(self.train_graph)
    #
    #     user = self.restore_user_e[interaction]
