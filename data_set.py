...
class TrainDate(Dataset):
    def __init__(self, user_count, item_count, all_dict=None, train_samples=None):
        self.user_count = user_count
        self.item_count = item_count
    ...
    def __getitem__(self, idx):
        ...
        total = []
        user, pos = self.train_samples[idx]
        inter = self.all_dict.get(str(user), None)
        neg = random.randint(1, self.item_count)
        while np.isin(neg, inter):
            neg = random.randint(1, self.item_count)
        return np.array([user, pos, neg])
...
class DataSet(object):
    ...
    def __get_behavior_items(self):
        ...
        self.train_interaction_list = [[int(key), val] for key, values in self.train_behavior_dict[self.behaviors[-1]].items() for val in values]
    ...
    def train_dataset(self):
        ...
        return TrainDate(self.user_count, self.item_count, self.train_behavior_dict['all'], self.train_interaction_list)
