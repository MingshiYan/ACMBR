...
class ACMBR(nn.Module):
    ...
    def _construct_graphs(self):
        ...
        self.target_graph = SGL(self.n_users + 1, self.n_items + 1, target_inter_matrix, self.device, self.embedding_size, self.layers, self.ssl_tau, self.ssl_weight)
        target_inter_matrix_bool = target_inter_matrix.astype(bool)

        for i in range(len(self.behaviors) - 1):
            self.confound_graphs.append(SGL(self.n_users + 1, self.n_items + 1, self.inter_matrix[i], self.device, self.embedding_size, self.layers, self.ssl_tau, self.ssl_weight))
            ...
            self.condition_graphs.append(SGL(self.n_users + 1, self.n_items + 1, tmp_target_inter_matrix, self.device, self.embedding_size, self.layers, self.ssl_tau, self.ssl_weight))
    ...
    def forward(self, batch_data):
        ...
        for idx in range(len(self.confound_graphs)):
            # 计算混淆概率
            tmp_loss, tmp_confound_user_all_embedding, tmp_confound_item_all_embedding = self.confound_graphs[idx].calculate(all_embeddings, user.long().squeeze(), p_item.long().squeeze(), n_item.long().squeeze(), if_calculate_loss=True)
            aux_loss.append(tmp_loss)

            # 计算条件概率
            tmp_condition_user_all_embedding, tmp_condition_item_all_embedding = self.condition_graphs[idx].calculate(all_embeddings, if_calculate_loss=False)

            # 为目标行为
            batch_target_confound_user_emb = tmp_confound_user_all_embedding[user.long()]
            batch_target_confound_p_item_emb = tmp_confound_item_all_embedding[p_item.long()]
            batch_target_confound_n_item_emb = tmp_confound_item_all_embedding[n_item.long()]
            ...
            batch_p_confound_score = torch.sum(batch_target_confound_user_emb * batch_target_confound_p_item_emb, dim=-1).squeeze()
            batch_n_confound_score = torch.sum(batch_target_confound_user_emb * batch_target_confound_n_item_emb, dim=-1).squeeze()
    ...
    def full_predict(self, users):
        if self.storage_user_embeddings is None:
            ...
            for idx in range(len(self.confound_graphs)):
                # 计算混淆概率
                tmp_confound_user_all_embedding, tmp_confound_item_all_embedding = self.confound_graphs[idx].calculate(all_embeddings, if_calculate_loss=False)
